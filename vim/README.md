...
仓库自有的 `.vimrc` 和根目录 `*.vim` 使用统一的 Vimscript 风格；`vendor/` 与
`colors/` 下的第三方或主题文件不参与整理。规则由 `vim/.editorconfig` 和
`.vimrc` 的 `FileType vim` 设置共同提供：

- 使用 2 个空格缩进，禁止 Tab、行尾空白，并保留 UTF-8、LF 和文件末尾换行。
- 代码行控制在 100 列以内；续行使用 Vimscript 的 `\`，保持表达式结构清晰。
- 条件、循环和异常处理使用完整的块结构，每行只放一个命令，不用 `|` 串联命令。
- 函数使用 `function! ... abort`，模块内部函数和变量保持脚本局部作用域。
- 字符串比较使用 `==#`、`!=#`、`=~#` 等大小写明确的运算符。
...
直接运行 `vim`，首页恢复 Vim 自带的初始欢迎文字，并在标题下加入 slogan：
`Les annees heureuses sont des annees perdues.`
版本号和发行版的修改者信息跟随当前 Vim；文字使用普通正文样式，无菜单、图标、下划线、背景高亮或页脚，也不注册首页专用按键。
...
欢迎文字块以整个 Vim 界面的中心为锚点，打开、关闭或调整文件树时保持屏幕位置稳定。
...
文件树沿用 nvimtree/neotree 的按键习惯，通过 netrw 的 `g:Netrw_UserMaps` 注册，
...
[fzf 的 reload 等待问题](https://github.com/junegunn/fzf/pull/4364) 超过 100 ms。
可用 `python3 vim/tests/search_latency.py --rounds 30 --max-ms 100`（在 Dotfiles 根目录运行）
复测真实按键到弹窗、结果、预览和恢复编辑的耗时。基准默认创建 1 万个临时文件，
保留已安装的 LSP，分别报告首次、热启动中位数与 P95；`--no-lsp` 可隔离语言服务器开销。
每次 `fp` 打开后还交替修改查询 6 次，覆盖持续使用后的刷新延迟。
`--max-ms 100` 检查所有样本（含首次），任一指标达到阈值即失败；
`--key-delay-ms 30` 模拟逐字输入，结果耗时从最后一个按键开始计算。
`--size 2097152` 可验证 2 MiB 大文件的轻量模式，基准结束后自动清理临时文件。
本机的前后对照、P95 和测量范围见 [PERFORMANCE.md](PERFORMANCE.md)。
